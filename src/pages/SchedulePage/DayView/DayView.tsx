...
export const DayView: FC = () => {
  ...
  const {
    ...
    type: `list&start_date=${moment(new Date()).format(
      'YYYY-MM-DD'
  ...
  const [exportcalender, {}] = useExportCalenderMutation();
  ...
  const [deleteDetailsDialog, setDeleteDetailsDialog] = useState(false);
  ...
  useEffect(() => {
    if (
      ...
      window.document.querySelector('.fc-timeGridDay-button')
    ) {
      (window.document.querySelector('.fc-timeGridDay-button') as any).click();
    }
  }, [viewType]);
  ...
  const handleEventClick = (clickInfo: any) => {
    setCurrentDetails({
      ...
      end: clickInfo.event.end.toString(),
  ...
  teamMemberOptions.unshift({ value: 'all', label: 'All Team Members' });
  ...
  //   var bodyFormData: any = new FormData();
  //   bodyFormData.append('export_ics', file);

  //   const result = await exportcalender(bodyFormData);
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <p className="sans-p m-b-8">
            ...
            {moment(currentDetails?.end).format('ddd,MMM D,yyyy')}
          </p>
          <p className="sans-p m-b-8">
            {moment(currentDetails?.startStr).format('HH:mm A')} -{' '}
            {moment(currentDetails?.endStr).format('HH:mm A')}
          </p>
        </DialogContent>
      </Dialog>
      ...
      <Modal
        ...
        width={'sm'}
        open={deleteDetailsDialog}
        closeIconVisible
        title={t('warning')}
        onClose={() => setDeleteDetailsDialog(false)}
      >
        <ErrorBoundary>
          <Typography css={tasksDeleteText} gutterBottom>
            {t('areYouSureYouWantToDeleteThisEvent')}
