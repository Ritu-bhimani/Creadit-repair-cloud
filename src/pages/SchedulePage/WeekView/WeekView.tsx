...
export const WeekView: FC = () => {
  ...
  const {
    ...
    type: `list&start_date=${moment(startWeek).format(
      'YYYY-MM-DD'
    )}&end_date=${moment(endWeek).format('YYYY-MM-DD')}`
  });
  ...
  useEffect(() => {
    if (
      ...
      window.document.querySelector('.fc-timeGridWeek-button')
    ) {
      (window.document.querySelector('.fc-timeGridWeek-button') as any).click();
    }
  }, [viewType]);
  ...
  const handleEventClick = (clickInfo: any) => {
    setCurrentDetails({
      ...
      endStr: clickInfo.event.endStr.toString()
    });
      ...
      >
        ...
        >
          ...
          <p className="sans-p m-b-8">
            ...
            {moment(currentDetails?.end).format('h:mm A')}
          </p>
        </DialogContent>
      </Dialog>
